...
for t=1:size(X,2)
    if(t==1)
        a_tl(:,:,t)=X(:,t)*w_il;  %每一个输入乘以对每个input gate的权值
        b_tl(:,:,t)=Sigmoid(a_tl(:,:,t));
        
        a_tf(:,:,t)=X(:,t)*w_if;  %每一个输入乘以对每个foget gate的权值
        b_tf(:,:,t)=Sigmoid(a_tf(:,:,t));
        
        a_tc(:,:,t)=X(:,t)*w_ic;  %每一个输入乘以对每个cell的权值
        s_tc(:,:,t)=b_tl(:,:,t).*Sigmoid2(a_tc(:,:,t));
        
        a_tw(:,:,t)=X(:,t)*w_iw + s_tc(:,:,t).*w_cw;  %每一个输入乘以对每个output gate的权值 + 每一个output gate对应所有cell的状态值
        b_tw(:,:,t)=Sigmoid(a_tw(:,:,t));
        
        b_tc(:,:,t)=b_tw(:,:,t).*Sigmoid3(s_tc(:,:,t));  %每个cell的输出 h函数
        
        a_tk(:,:,t)=b_tc(:,:,t)*w_ck;  %输出
