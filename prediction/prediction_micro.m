function [ output_args ] = prediction_micro( filenew, fileold )

%read data
[~,~,x0a,x1a,x2a,x3a,~,~] = textread(filenew,'%s%s%d%d%d%d%d%s','headerlines',1);
for i=1:length(x0a)
    if mod(i,2)==1
        y0((i+1)/2)=x0a(i);
        y1((i+1)/2)=x1a(i);
...
[~,~,x0b,x1b,x2b,x3b,~,~] = textread(fileold,'%s%s%d%d%d%d%d%s','headerlines',1);
for i=1:length(x0b)
    if mod(i,2)==1
        y0((i+1)/2+tmp)=x0b(i);
        y1((i+1)/2+tmp)=x1b(i);
